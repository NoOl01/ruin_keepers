...
const TourDetail: React.FC = () => {
    ...
    useEffect(() => {
        axios.get(`${baseUrl}/api/v1/tours/tourById?id=${id}`)
    ...
    return (
        <div className="container mx-auto p-6">
            <div className="flex flex-col lg:flex-row">
                <img src={`${baseUrl}${tour.Image}`} alt={tour.Name} className="w-full lg:w-1/2 h-64 object-cover rounded-md" />
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mt-4">
                {tour.Points.map((point) => (
                    <div key={point.ID} className="border rounded-lg p-4 shadow-md">
                        <img src={`${baseUrl}${point.Image}`} alt={point.Name} className="w-full h-48 object-cover rounded-md" />
